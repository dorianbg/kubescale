...
class MemoryUtilisation(MetricSource):
    def __init__(self, prom_url: str, step_size_secs: int, deployment_name: str):
        self.metric_name = "memory_usage_per_pod"
        self.pod_name = deployment_name + "-.*"
        self.query_template = """
            sum(eagle_pod_container_resource_usage_memory_bytes{{ pod=~"{pod_name}", container!="POD", container=~"{container_name}", phase="Running"}}) by ({by_keyword})         
            / sum(eagle_pod_container_resource_limits_memory_bytes{{ pod=~"{pod_name}", container!="POD", container=~"{container_name}", phase="Running"}}) by ({by_keyword})"""
        # example
        #             sum(eagle_pod_container_resource_usage_memory_bytes{ pod=~"headlesswebapp.*", container!="POD", container=~"headlesswebapp" })
        #             / (
        #                 sum(eagle_pod_container_resource_requests_memory_bytes{ pod=~"headlesswebapp.*", container!="POD", container=~"headlesswebapp" }) /
        #                 count(count by (pod) (eagle_pod_container_resource_usage_memory_bytes{ pod=~"headlesswebapp.*", container!="POD", container=~"headlesswebapp" }))
        #             ) /
        #                 count(count by (pod) (eagle_pod_container_resource_usage_memory_bytes{ pod=~"headlesswebapp.*", container!="POD", container=~"headlesswebapp" }))
